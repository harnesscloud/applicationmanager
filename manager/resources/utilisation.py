...
class Monitor:
    ...
    def convert2percentage(self, proc_data, tot_data):
        result = []
    ...
    def get_monitoing(self):
        ...
        for addr in info:
            ...
            # cpu_util = self.convert2percentage(info[addr]['CPU_U_S_TIME'], info[addr]['CPU_TOT_TIME'])
            # memory_util = self.convert2percentage(info[addr]['MEM_U_S_BYTE'], info[addr]['MEM_TOT_BYTE'])
            # for attr in info[addr]:
            #     del info[addr][attr] 
            # info[addr]['Cores'] = cpu_util
            # info[addr]['Memory'] = memory_util

        self.get_recommendation(info)
