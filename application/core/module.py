...
from utils.mondata import UtilizationDataThread
from utils.resources import get_utilization
...
class ModelManagement:
    ...
    def optimize_objectives(self, execution):
	    ...
	    for comb in MathUtils.generate_combinations(indices):
		conf = {}
		#rebuild conf
...
class Implementation(Base, ModelManagement):
    accepted_params = [
            {
    ...
    def evaluateConstraints(self, variables = {}):
        ...
        for expr in self.GlobalConstraints:
            ...
            for var in variables:
                constraint = constraint.replace(var, str(variables[var]))
            if not eval(constraint):
                #print "Constraint %s is not satisfied for %s."% (expr, str(variables))
                return False
        
        return True
    

class Module(Base, ModelManagement):
    accepted_params = [
                        {
                        'name' : 'ModuleName',
                        'type' : str,
